	...
	public class AlertService : IAlertService
	{
		...
		public async Task<AlertResponse> ApproveRequestFromAlertAsync(ApproveRequestFromAlertRequest request, int approverUserId)
		{
			try
			{
				...
				var eventId = alert.EventId;
				var eventName = alert.Title ?? EventNameConstants.Request;
			...
			try
			{
					...
					eventNameLower.Contains(EventNameConstants.CancelLeave.ToLower()))
				{
					...
					else
					{
						...
						var leaveResult = await _leaveService.RejectLeaveRequestAsync(requestId.Value, rejecterUserId, null);
				...
				var updateSuccess = await _repo.RejectAlertAsync(request.AlertId, rejecterUserId, null);
				if (!updateSuccess)
				{
