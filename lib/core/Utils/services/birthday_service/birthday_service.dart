...
class BirthdayService {
  static Future<List<Birthday>?> getUpcomingBirthdays() async{
    try{
      final token = await TokenStorage.getToken();
      if(token == null) return null;

      final uri = Uri.parse(BaseUrls.upBirthday);
      print('BIRTHDAY SERVICE : CALLING STANDARD API => $uri');
      ...
      if (decoded is Map && decoded ['success'] == true){
        ...
        return data.map((json) => Birthday.fromJson(json)).toList();
      }
      return null;
    } catch (e) {
      print('BIRTHDAY SERVICE ERROR (standard) => $e');
      return null;
    }
  }
}