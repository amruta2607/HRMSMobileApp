...
class AwardService {
  static Future<List<Award>?> getUpcomingAwards() async {
    try{
      final token = await TokenStorage.getToken();
      if (token == null) return null;

      final uri = Uri.parse(BaseUrls.upAwards);
      print('AWARDS SERVICE: CALLING STANDARD API => $uri');
      ...
      if(decoded is List){
        return decoded.map((json) => Award.fromJson(json as Map<String, dynamic>)).toList();
      }
      return null;
    }catch (e) {
      print('AWARD SERVICE ERROR => $e');
      return null;
    }
  }
}